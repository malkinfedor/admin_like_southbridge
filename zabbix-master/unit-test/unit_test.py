...
zbx_group_get = zapi.hostgroup.get(
    {
        "search":
            {
                "name": "autotest.slurm.io"
            }
    }
)

if not zbx_group_get:
    print "Group autotest.slurm.io not found. Test failed"
    raise SystemExit(1)
...
zbx_host_get = zapi.host.get(
    {
        "search":
            {
                "name": "icmp_rus.autotest.slurm.io"
            }
    }
)

if not zbx_host_get:
    print "Host icmp_rus.autotest.slurm.io not found. Test failed"
    raise SystemExit(1)
...
zbx_host_get = zapi.host.get(
    {
        "search":
            {
                "name": "content_check.autotest.slurm.io"
            }
    }
)

if not zbx_host_get:
    print "Host content_check.autotest.slurm.io not found. Test failed"
    raise SystemExit(1)
else:
...
if not zbx_usergroup_get:
    print "UserGroup autotest.slurm.io not found. Test failed"
    raise SystemExit(1)
...
zbx_user_get = zapi.user.get(
    {
        "search":
            {
                "alias": "autotest.slurm.io"
            }
    }
)

if not zbx_user_get:
    print "User autotest.slurm.io not found. Test failed"
    raise SystemExit(1)
